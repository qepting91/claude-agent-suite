...
@pytest.mark.parametrize("word_count,expected_tokens", [
    (0, 0),
    (1, 1),
    (10, 13),
    (100, 130),
...
def test_required_frontmatter_fields():
    ...
    missing_valid = [field for field in required_fields if field not in frontmatter_valid]
    assert len(missing_valid) == 0

    # Check invalid frontmatter
    missing_invalid = [field for field in required_fields if field not in frontmatter_invalid]
